...
프로젝트 문서를 지정된 순서대로 읽고, 요청 범위를 고정한 뒤, 허용된 계층만 구현한다. 작업 종료 조건은 `CHANGELOG.md` 업데이트와 `./scripts/verify.sh all` 통과다.
...
2. 필수 문서를 순서대로 읽는다.
3. 요청된 도메인 패키지만 구현한다.
4. `./scripts/install-git-hooks.sh`와 `.githooks` 기준 훅 구성을 확인한다.
5. `CHANGELOG.md`는 현재 작업 버전 아래만 수정하고 새 버전을 임의로 만들지 않는다.
6. `./scripts/verify.sh all`을 실행한다.
7. 수정 파일, 검증 결과, 문서상 누락된 사항을 함께 보고한다.
...
- service 패키지는 오케스트레이션 서비스와 보조 정책/이력/동기화 서비스로 분리할 수 있다.
...
- 인증 저장 모델이 auth 패키지에 있더라도, 사용자 자기관리 API는 user 패키지 controller/service가 노출할 수 있다.
