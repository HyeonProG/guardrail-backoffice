...
- API 요청 중 access token 만료로 `401`이 발생하면 `sessionId`, `refreshToken`으로 토큰을 재발급한 뒤 원 요청을 1회 재시도합니다.
- 로그아웃은 `/api/v1/auth/logout`에 `sessionId`, `refreshToken`을 전달한 뒤 저장된 인증 정보를 삭제합니다.
