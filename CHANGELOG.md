...
## [1.0.4]
### 수정
- 백오피스 전역 UI를 민트/네이비 기반 라이트·다크 테마 구조로 정리
- 로그인 화면, 사이드바, 대시보드, 상품 상세, 사용자 관리 화면의 레이아웃과 액션 버튼 스타일을 보정
- 대시보드 KPI 카드 이동 흐름과 사용자 관리 테이블 overflow 표시를 개선

## [1.0.3]
### 수정
- 승인 완료 상품을 관리자/운영자가 다시 승인 대기 또는 반려 상태로 변경할 수 있도록 상태 전이 정책을 확장
- 사용자 단건 조회와 기본 정보 수정 계약을 본인 이메일/이름 수정 기준으로 정리
- 카테고리 순환 부모 지정 방지, 상품 요청 검증, 상품 파일 첨부 권한 검증을 보강
- 상품 옵션 요청 DTO에서 사용하지 않는 actorId를 제거하고 프론트 요청 payload와 문서를 동기화
- AI 상품 설명 생성 프롬프트를 짧은 키워드도 구매 설득형 설명으로 확장하도록 보강

## [1.0.2]
### 수정
- 사용자 기본 정보 수정을 본인 이메일과 이름 수정 용도로 제한
- 로그아웃 시 refresh token 소유 검증을 추가하고 철회 세션의 access token 사용을 차단
- access token 만료 시 자동 재발급 흐름을 추가
- 인증 보안 규칙과 로그아웃 명세 문서를 현재 구현 기준으로 정리

## [1.0.1]
### 수정
- API 명세, 도메인 문서, 하네스 문서, 훅 런북을 현재 코드와 하네스 규칙 기준으로 다시 정합화
- 사용자 API 명세를 실제 공개 엔드포인트 기준으로 보강하고 ArchUnit 허용 범위를 문서에 명시

## [1.0.0]
### 수정
- 상품, 승인 요청, 카테고리, 상품 옵션 관리 화면의 UX와 액션 흐름을 최종 정리
- 프론트 라우트 코드 스플릿과 대시보드 구성을 정리하고 주요 화면 UI를 전반 리뉴얼
- 인증과 사용자 책임을 분리하고, 상품 서비스의 상태 전이·선택 옵션 동기화·이력 기록 책임을 분리

## [0.9.9]
### 수정
- 프론트 전역 UI 톤과 주요 관리 화면 디자인을 전면 정리
- 공통 모달 스타일을 단순화하고 바깥 클릭 닫힘, 화면 중앙 고정 표시로 보정

## [0.9.8]
### 수정
- 상품 도메인과 상품 등록/수정 화면에서 수량 필드를 제거
- 상품 상세, 승인 요청 상세, API 명세, DDL 문서를 현재 상품 구조에 맞게 정리
- 옵션값과 상품 선택 스냅샷의 `additional_price` 컬럼 제거 기준으로 DDL 문서를 정리

## [0.9.7]
### 수정
- 카테고리 관리 목록을 최상위/하위 depth 탐색 구조로 변경하고 breadcrumb와 뒤로가기 흐름을 추가
- 상품 생성과 상품 수정의 카테고리 선택 모달을 depth 탐색 기반으로 정리
- 카테고리 선택과 하위 이동 UX를 현재 계층형 카테고리 구조에 맞게 보정

## [0.9.6]
### 수정
- 옵션값과 상품 선택 스냅샷에서 추가 금액 필드를 제거하고 관련 백엔드 DTO, 서비스, 프론트 타입을 정리
- 옵션값 상세, 선택 항목 생성/수정, 상품 등록/수정/상세 화면에서 추가 금액 입력과 노출을 제거
- 상품 옵션 API 명세와 도메인 문서를 현재 구조에 맞게 정리

## [0.9.5]
### 추가
- 공통 번호 이동이 가능한 프론트 페이지네이션 컴포넌트를 추가
### 수정
- 카테고리 생성 완료 후 목록 첫 페이지로 돌아와 새 목록을 바로 확인하도록 흐름을 보정
- 카테고리, 상품 옵션, 선택 항목, 상품 목록, 승인 완료 상품 목록의 페이지 이동 UI를 번호 기반으로 정리

## [0.9.4]
### 추가
- 공통 성공 응답 직렬화 규약이 `isSuccess`와 내부 상태 코드 비노출 기준을 유지하도록 테스트를 추가
### 수정
- 공통 성공 응답 JSON 구조를 프론트 타입과 API 문서 기준에 맞게 정리
- 로그인, 로그아웃, 카테고리 조회 규칙과 현재 배포 스택 기준으로 주요 문서를 다시 정합화
- Swagger 설명에서 남아 있던 `soft delete`와 내부 상태 표현을 사용자 관점 문구로 정리
### 삭제
- 현재 Vercel, Render 배포 구조에서 사용하지 않는 `docker-compose.yml`, `frontend`용 Docker/Nginx 설정 파일 정리
...
- 로그인 직후 헤더와 레이아웃의 사용자 정보 조회가 실패해도 즉시 세션 만료로 오인하지 않도록 프론트 인증 리다이렉트 흐름을 보정
