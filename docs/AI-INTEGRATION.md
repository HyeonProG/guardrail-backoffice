...
- 업로드된 이미지를 AI 입력으로 사용하지 않는다.
- AI가 추가 이미지를 생성하거나 이미지 위에 설명 문구를 합성하지 않는다.

## 3. 실행 기준
### 3.1 기본 실행 대상
기본 AI 실행 대상은 `OpenAI API`다.

규칙:
- 구현 범위는 `Spring AI + OpenAI` 조합으로 고정한다.
...
- 기본 프론트 UX에서는 `optionSummary`를 비워서 전달하며, 선택 항목은 AI 입력에 사용하지 않는다.
- 이미지 URL, 파일 경로, 바이너리 데이터는 입력값에 포함하지 않는다.

## 7. 프롬프트 구성 기준
- 프롬프트는 상품 설명 생성 목적에만 사용한다.
- 상품명, 카테고리, 옵션 요약, 특징 키워드를 구조화해 전달한다.
- 특징 키워드가 짧게 입력되어도 사용 장면, 고객 관점의 장점, 구매 설득 문장으로 자연스럽게 확장한다.
- 과장 표현, 허위 성능, 확인되지 않은 효능 표현은 금지한다.
- 입력에 없는 성능, 효능, 소재, 브랜드, 인증 정보는 임의로 추가하지 않는다.
...
- AI 응답은 별도 초안 테이블에 저장하지 않고 상품의 `description` 필드에 바로 반영한다.
