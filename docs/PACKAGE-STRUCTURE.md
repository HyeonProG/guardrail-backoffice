...
- 공통화보다 도메인 응집도를 우선한다.
- 외부 AI provider 호출, 프롬프트 조합, AI 응답 매핑은 `common.ai`에 둔다.
- 도메인 서비스는 AI provider 구현체가 아니라 `common.ai` 인터페이스만 호출한다.
...
- `ProductContentDraft`, `ProductContentHistory` -> `productcontent`
