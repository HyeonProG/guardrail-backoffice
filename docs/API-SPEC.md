...
- 현재 로그인 사용자 본인만 요청할 수 있다.
- 사용자 수정 API는 `email`, `name`만 변경한다.
- `role`, `status`는 변경하지 않는다.
- `email`은 다른 사용자와 중복될 수 없다.
...
- 특징 키워드가 짧게 입력되어도 사용 장면과 고객 관점의 장점으로 확장해 구매 설득형 설명을 생성한다.
- 입력에 없는 효능, 성능, 소재, 브랜드, 인증 정보는 임의로 추가하지 않는다.
